                ...
                <tr v-for="(Item,index) in item.orderDetailList" :key="Item.id">
                    ...
                    <template v-if="index===0">
                        ...
                        <td :rowspan="item.orderDetailList.length" width="8%" class="center">
                            <a href="javascript:" class="btn">{{ item.orderStatusName }}</a>
                        </td>
                        <td :rowspan="item.orderDetailList.length" width="13%" class="center">
                            <ul class="unstyled">
                                <li>
...
export default {
    ...
    methods: {
        ...mapActions('order', ['get_my_order_info']),
        formFeed(val) {
            if (val === this.page) {
                return
            }
    ...
    computed: {
        ...mapGetters('order', ['records', 'total']),

    },
    mounted() {
        console.log(this.limit, this.page)
        this.get_my_order_info({page: this.page, limit: this.limit})
    },
    watch: {
        page: {
            handler() {
